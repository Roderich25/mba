...
def build_model(hl, af, l2, do=0.5):
    model = Sequential()
    ###
    ...
    ###
    model.add(coral.CoralOrdinal(3))  # .25 .35
    ...
    for af in ['tanh']:
        for l2 in [0]:  # , 0.0001, 0.001, 0.01, 0.1]:
            for do in [0.5]:
                ...
                print(f1_mac)
                print(
                    f'### hl:{hl}, af:{af}, l2:{l2}, do:{do}, acc:{np.mean(ord_acc)}, f1:{np.mean(f1_mac)}, acc:{np.mean(ord_acc_2)}, f1:{np.mean(f1_mac_2)}')
