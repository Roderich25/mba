...
def main(val_map=False):
    ...
    for hl in [24]:
            ...
            for l2 in [0.001]:
                for nn in [3]:
                    ...
                    for train, test in kf.split(Xtrain, np.sum(ytrain, axis=1)):
                        ...
                        model = build_model(hl, af, l2, X_val.shape[1], nn)
                        tf.random.set_seed(0)
                        history = model.fit(part_X_train, part_y_train, verbose=0, epochs=400,
        ...
        # hl:24, af:tanh, l2:0.001, nn:3, acc:0.7486005089058524+/-0.022380288475619375,
        # f1:0.7252899993197212+/-0.02334332577877526, mae:0.26208651399491095+/-0.020228607546414035
                ...
                for l2 in [0.001]:
                    for nn in [3]:
                        ...
                        for train, test in kf.split(Xtrain, np.sum(ytrain, axis=1)):
                            ...
                            y_pred = np.argmax(model.predict(X_), axis=1) + 1
