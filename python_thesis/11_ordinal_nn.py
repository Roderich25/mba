...
for dn in [3]:
    ...
    rezago_social = rezago[["clase2015", "Key", "POB_TOTAL", "LAT", "LON"]]
    df = pd.merge(rezago_social, denue_wide, on=['Key'])
    df = df[df['clase2015'] > 0]
    y_original = df['clase2015'].map({1: 3, 2: 2, 3: 1})
    ...
    df.drop(["clase2015", "Key", "LAT", "LON"], axis=1, inplace=True)
        ...
        for af in ['sigmoid', 'relu', 'tanh']:
            for l2 in [0, 0.0001, 0.001, 0.01, 0.1]:
                for nn in [1, 2, 3]:
                    ...
                    # print(ord_acc)
                    # print(f1_mac)
                    print(f'# dn:{dn}, nn:{nn}, hl:{hl}, af:{af}, l2:{l2}, acc:{np.mean(ord_acc)}, f1:{np.mean(f1_mac)}')
